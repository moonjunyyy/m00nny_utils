...
class _Thread_Pool:
    ...
    @classmethod
    def set_max_thread(
        cls, max_thread: int) -> None: cls.__max_running_threads = max_thread
    ...
    def __init__(self) -> None:
        ...
        # Map the POSIX thread id to the thread id.
        self.__posix_id_to_tid = {}
        self.__thread_queue = []          # Thread queue.
        # Condition variable for the thread queue.
        self.__queue_cv = Condition()
    ...
    def set_max_thread(
        self, max_thread: int) -> None: self.__max_running_threads = max_thread
    ...
    def register(self, owner_id, group=None, target=None, name=None, daemon=False, args=(), kwargs={}) -> str:
        ...
        __owner_id = self.__posix_id_to_tid[owner_id] if owner_id in self.__posix_id_to_tid.keys(
        ) else ""
        # Generate a random 4 character Hexadecimal string as the suffix of the thread id.
            ...
            try:
                with self.__queue_cv:
                    if len(self.__thread_queue) == 0:
                        # Wait for the thread queue to be not empty.
                        self.__queue_cv.wait()
                        # Sort the thread queue by the thread depth and lifetime.
                        self.__thread_queue.sort()
                ...
                print(f"Thread manager raised an exception: {
