...
class PathNode:
    ...
    def get(self, name: str) -> _Node:
        ...
        with self.children_lock:
            for _child in self.children:
                if _child == name: _found = _child; break
        return _found
    ...
    def merge(self, dst: _Node | str) -> None:
        if isinstance(dst, str): dst = PathTree(path=dst, depth_limit=self.depth_limit)
        for _child in self.children: _child.mv(dst)
        self.rm()
    ...
    def mv(self, dst: _Node | str) -> None:
        ...
        if self.name in dst.children:
            ...
            if self.type == PathNode.Type.DIRECTORY:
                ...
                with self.children_lock:
                    for _child in self.children:
                        _th_list.append(Thread(target=_child.mv, args=(_subnode,), daemon=True))
                for _th in _th_list: _th.start()
                for _th in _th_list: _th.join()
                self.rm()
                return
            else:
                log.warning(
    ...
    def rm(self) -> None:
        if self.type == PathNode.Type.DIRECTORY:
            ...
            try:
                ...
                if self.parent: self.parent.remove(node=self)
        ...
        with self.children_lock:
            for _child in self.children:
                if _child.type == PathNode.Type.DIRECTORY:
                    _th_list.append(Thread(target=_child.find, args=(pattern,), daemon=True))
        for _th in _th_list: _th.start()
        for _th in _th_list: _ret += _th.join()
        with self.children_lock:
            for _child in self.children:
                if   pattern == _child.name: _ret.append(_child)
                elif pattern in _child.name: _candidates.append(_child)
        if len(_ret) == 0: _ret += _candidates
        return _ret

    def parse(self) -> None:
        if self.depth == self.depth_limit: return
        if self.type == PathNode.Type.DIRECTORY:
