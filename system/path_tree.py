...
class PathNode:
    ...
    def __mv(self, dst: _Node | str) -> None:
        ...
        if self.name in dst.children:
            ...
            if self.type == PathNode.Type.DIRECTORY:
                ...
                _th_list = []
                for _child in self.children:
                    _th_list.append(Thread(target=_child.__mv, args=(_subnode,), daemon=True))
                    _th_list[-1].start()
    ...
    def __find(self, pattern: str) -> List[_Node]:
        ...
        for _child in self.children:
            if _child.type == PathNode.Type.DIRECTORY:
                _th_list.append(Thread(target=_child.__find, args=(pattern,), daemon=True))
                _th_list[-1].start()
        for _th in _th_list:
            _ret += _th.join()
        for _child in self.children:
    ...
    def parse(self) -> None:
        ...
        else: self.size = os.path.getsize(self.path)
