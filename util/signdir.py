...
def validate_signed_file(
    file_path: str, hash_type: str = "sha256", hash_file_path: str = None
) -> None:
    import os

    if hash_file_path is None:
        hash_file_path = os.path.join(file_path, ("." + hash_type))
...
def validate_signed_dir(
    directory_path: str, hash_type: str = "sha256", hash_file_path: str = None
) -> None:
    import os

    if hash_file_path is None:
        hash_file_path = os.path.join(directory_path, ("." + hash_type))
    hash_dict = {}
    try:
        if not os.path.isfile(hash_file_path):
            raise FileNotFoundError(f"Hash file not found: {hash_file_path}")
        ...
        for _path in path_tree:
            if _path.name == ("." + hash_type):
                continue
            if _path.is_file() and _path.rel_path in hash_dict:
                log.debug(f"Verifying: {_path.rel_path}")
                if get_file_hash(
                    file_path=_path.path, hash_type=hash_type
                ) != hash_dict.pop(_path.rel_path, None):
                    raise ValueError(
                        f"{_path.rel_path} expected hash: {hash_dict[_path.rel_path]} but got different hash"
                    )
        if len(hash_dict) > 0:
            raise ValueError(f"Following files are missing: {hash_dict.keys()}")
        return True
    except Exception as e:
        log.warning(
            f"Error occurred while verifying the integrity of the directory:\n{e}"
        )
    return False
...
def sign_dir(
    ...
    with open(hash_file_path, "w") as f:
        ...
        for _path in path_tree:
            ...
            if _path.is_file():
                f.write(
                    f"{_path.rel_path}: {get_file_hash(file_path=_path.path, hash_type=hash_type)}\n"
                )
    log.debug(f"Integrity signature saved: {hash_file_path}")
