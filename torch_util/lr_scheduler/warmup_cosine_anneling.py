...
class WarmUpCosineAnnelingScheduler(torch.optim.lr_scheduler.LRScheduler):
    ...
    def get_lr(self):
        last_epoch = (self.last_epoch - self.warmup_steps) % (self.t_total - self.warmup_steps)
        if self.last_epoch < self.warmup_steps:
            return [self.initial_lrs[i] * self.last_epoch / self.warmup_steps for i in range(len(self.optimizer.param_groups))]
        else:
            return [0.5 * (1 + math.cos(math.pi * last_epoch / (self.t_total - self.warmup_steps))) * self.initial_lrs[i] for i in range(len(self.optimizer.param_groups))]